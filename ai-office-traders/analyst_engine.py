...
def _get_shared_market_context(prices: list[dict]) -> dict:
    ...
    context = {"pairs": [], "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    for p in prices:
...
def analyst_work(analyst, prices: list[dict]) -> dict:
    ...
    elif analyst.department == "Сентимент и Новостной анализ":
        report["findings"] = _sentiment_analysis(analyst, prices)
    elif analyst.department == "Прогнозирование":
        report["findings"] = _forecast_analysis(analyst, prices)
    elif analyst.department == "Алгоритмическая торговля":
        report["findings"] = _algo_trading_analysis(analyst, prices)
    elif analyst.department == "Управление портфелем":
        report["findings"] = _portfolio_analysis(analyst, prices)
    elif analyst.department == "Тестирование стратегий":
        report["findings"] = _backtest_analysis(analyst, prices)
    elif analyst.department == "Поведенческие финансы":
        report["findings"] = _behavioral_analysis(analyst, prices)
    elif analyst.department == "Волатильность":
        report["findings"] = _volatility_analysis(analyst, prices)
    elif analyst.department == "Фундаментальный анализ":
        report["findings"] = _fundamental_analysis(analyst, prices)
    else:
        report["findings"] = []
...
def _ict_smc(analyst, prices):
    ...
    for p in prices:
        ...
        if analyst.id == 27:
            ...
            finding["order_flow"] = "Анализ ордерфлоу",
            finding["accumulation_zones"] = "Мониторинг",
            finding["signal"] = "NEUTRAL"
    ...
    if analyst.id == 30:
        ...
        for p in prices[:3]:
            ...
            if df.empty or len(df) < 10:
                continue
            close = df["Close"]
            chg = (float(close.iloc[-1]) - float(close.iloc[-5])) / float(close.iloc[-5]) * 100
            findings.append({
                "pair": p["name"],
                "macro_change_5d": round(chg, 2),
                "signal": "NEUTRAL",
            })

    elif analyst.id == 64:
        findings.append({
            "central_bank_policy": "Мониторинг ФРС/ЕЦБ/Банк Англии",
            "rate_outlook": "Ставки стабильны",
            "signal": "NEUTRAL",
        })
...
def _quant_analysis(analyst, prices):
    ...
    if analyst.id == 8:
        ...
        findings.append({
            "quant_strategy": "Количественные модели активны",
            "backtest_results": "В норме",
            "signal": "NEUTRAL",
        })
...
def _risk_analysis(analyst, prices):
    ...
    if analyst.id == 10:
        ...
        findings.append({
            "position_sizing": "Оптимальный",
            "risk_exposure": "В пределах нормы",
            "signal": "NEUTRAL",
        })
...
def _cross_market_analysis(analyst, prices):
    ...
    if analyst.id == 14:
        ...
        for p in prices[:3]:
            ...
            findings.append({
                "pair": p["name"],
                "cross_correlation": round(chg, 2),
                "signal": "NEUTRAL",
            })

    elif analyst.id == 71:
        findings.append({
            "stat_arb": "Пары в норме",
            "cointegration": "Мониторинг",
            "signal": "NEUTRAL",
        })
...
def _research_analysis(analyst, prices):
    ...
    if analyst.id == 19:
        ...
        findings.append({
            "alternative_data": "Спутниковые данные обновлены",
            "geolocation": "Мониторинг",
            "signal": "NEUTRAL",
        })
        ...
        for p in prices[:3]:
            ...
            findings.append({
                "pair": p["name"],
                "strategy_trend": "UP" if chg > 0.5 else "DOWN" if chg < -0.5 else "SIDEWAYS",
                "signal": "NEUTRAL",
            })
...
def _ops_analysis(analyst, prices):
    ...
    if analyst.id == 18:
        ...
        findings.append({
            "execution_quality": "Исполнение в норме",
...
def _tech_ops_analysis(analyst, prices):
    ...
    if analyst.id == 20:
        ...
        findings.append({
            "ml_models": "ML модели обновлены",
            "accuracy": "Мониторинг",
            "signal": "NEUTRAL",
        })
    elif analyst.id == 68:
        findings.append({
            "dashboards": "Все дашборды активны",
            "data_freshness": "Актуально",
            "signal": "NEUTRAL",
        })
...
def _sentiment_analysis(analyst, prices):
    findings = []
    ...
    if analyst.id == 50:
        for p in prices[:3]:
            ...
            close = df["Close"]
            chg = (float(close.iloc[-1]) - float(close.iloc[-5])) / float(close.iloc[-5]) * 100
            sentiment = "POSITIVE" if chg > 0.3 else "NEGATIVE" if chg < -0.3 else "NEUTRAL"
            findings.append({
                "pair": p["name"],
                "sentiment": sentiment,
                "price_change_5d": round(chg, 2),
                "signal": "BUY" if sentiment == "POSITIVE" and chg > 0.5 else "SELL" if sentiment == "NEGATIVE" and chg < -0.5 else "NEUTRAL",
            })
    elif analyst.id == 51:
        findings.append({
            "news_impact": "Мониторинг новостей",
            "upcoming_events": "NFP, CPI, Rate Decision",
            "signal": "NEUTRAL",
        })
    elif analyst.id == 72:
        findings.append({
            "cot_report": "Анализ COT позиций",
            "retail_bias": "Нейтральный",
            "signal": "NEUTRAL",
        })
...
def _forecast_analysis(analyst, prices):
    findings = []
    ...
    if analyst.id == 56:
        for p in prices[:3]:
            ...
            if df.empty or len(df) < 30:
                continue
            close = df["Close"]
            mean_ret = float(close.pct_change().mean()) * 100
            forecast = "BULLISH" if mean_ret > 0.1 else "BEARISH" if mean_ret < -0.1 else "NEUTRAL"
            findings.append({
                "pair": p["name"],
                "forecast": forecast,
                "expected_return": round(mean_ret, 3),
                "signal": "BUY" if forecast == "BULLISH" and mean_ret > 0.2 else "SELL" if forecast == "BEARISH" and mean_ret < -0.2 else "NEUTRAL",
            })

    return findings


def _algo_trading_analysis(analyst, prices):
    findings = []

    if analyst.id == 57:
        findings.append({
            "rl_agent": "RL-агент активен",
            "strategy": "Реинфорсмент-обучение",
            "signal": "NEUTRAL",
        })
    elif analyst.id == 69:
        findings.append({
            "hft_status": "HFT стратегии активны",
            "latency": "Низкая",
            "signal": "NEUTRAL",
        })
...
def _portfolio_analysis(analyst, prices):
    findings = []

    if analyst.id == 58:
        findings.append({
            "portfolio_status": "Портфель сбалансирован",
            "diversification": "Норма",
            "signal": "NEUTRAL",
        })
    elif analyst.id == 59:
        findings.append({
            "allocation": "Оптимальная аллокация",
            "rebalance": "Не требуется",
            "signal": "NEUTRAL",
        })
...
def _backtest_analysis(analyst, prices):
    findings = []

    if analyst.id == 60:
        findings.append({
            "backtest_status": "Бэктестинг завершён",
            "strategies_tested": "5 стратегий",
            "signal": "NEUTRAL",
        })
    elif analyst.id == 61:
        findings.append({
            "monte_carlo": "Стресс-тест пройден",
            "walk_forward": "Анализ завершён",
            "signal": "NEUTRAL",
        })
...
def _behavioral_analysis(analyst, prices):
    findings = []

    if analyst.id == 62:
        findings.append({
            "market_psychology": "Нейтральная",
...
def _volatility_analysis(analyst, prices):
    findings = []
...
def _fundamental_analysis(analyst, prices):
    findings = []
    ...
    if analyst.id == 70:
        for p in prices[:3]:
            ...
            close = df["Close"]
            chg_20 = (float(close.iloc[-1]) - float(close.iloc[-20])) / float(close.iloc[-20]) * 100
            trend = "BULLISH" if chg_20 > 1 else "BEARISH" if chg_20 < -1 else "NEUTRAL"
            findings.append({
                "pair": p["name"],
                "fundamental_trend": trend,
                "price_change_20d": round(chg_20, 2),
                "signal": "BUY" if trend == "BULLISH" and chg_20 > 2 else "SELL" if trend == "BEARISH" and chg_20 < -2 else "NEUTRAL",
            })
...
def global_meeting(dept_reports: list[dict]) -> dict:
    ...
    if LLM_ENABLED:
        try:
            ...
            if not LLM_FALLBACK:
                raise
            print(f"[WARN] LLM global meeting failed: {e}")

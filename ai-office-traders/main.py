...
from config import INSTRUMENTS, TIMEFRAME, ANALYSIS_INTERVAL_MINUTES, LLM_ENABLED
...
def display_prices(prices):
    table = Table(title=f"Gold Live [{TIMEFRAME}]", box=box.ROUNDED)
    ...
    for p in prices:
        chg = p.get('price_change', 0)
        chg_color = "green" if chg >= 0 else "red"
        table.add_row(p["name"], f"{p['price']:.2f}", f"{p['open']:.2f}", f"{p['high']:.2f}", f"{p['low']:.2f}", f"[{chg_color}]{chg:+.1f}p[/]")
    console.print(table)
    ...
    for r in reports:
        ...
        for f in r["findings"]:
            ...
            if f.get("llm_analysis"):
                parts.append(f"LLM: {str(f['llm_analysis'])[:80]}")
                if f.get("llm_confidence"):
                    parts.append(f"conf={f['llm_confidence']:.0%}")
            else:
                for k, v in f.items():
                    if k in ("signal", "pair", "llm_analysis", "llm_confidence", "llm_key_levels", "llm_model"):
                        continue
            ...
            findings.append(" | ".join(str(x) for x in parts))
        text = "\n".join(findings) if findings else "—"
        head_mark = " ★" if r.get("is_head") else ""
        llm_mark = " [LLM]" if r.get("llm_used") else ""
        table.add_row(str(r["analyst_id"]), r["analyst_name"] + head_mark + llm_mark, r["department"], text)
...
def display_global_report(report):
    ...
    if report.get("llm_recommendation"):
        lines.append(f"")
        lines.append(f"[bold magenta]LLM рекомендация:[/] {report['llm_recommendation']}")
    if report.get("llm_risk_note"):
        lines.append(f"[bold red]LLM риск:[/] {report['llm_risk_note']}")
...
def run_cycle():
    ...
    console.print(f"\n[bold]{'='*60}[/bold]")
    console.print(f"[bold]  AI OFFICE — GOLD TRADING @ {ts} | TF: {TIMEFRAME}[/bold]")
    llm_status = "[bold green]LLM: Mistral[/]" if LLM_ENABLED else "[bold yellow]LLM: OFF[/]"
    console.print(f"[bold]  {llm_status}[/bold]")
    console.print(f"[bold]{'='*60}[/bold]\n")

    console.print("[bold cyan]1. Получение цен золота...[/bold cyan]")
    ...
    console.print("[bold cyan]2. Работа аналитиков золота...[/bold cyan]")
    from analyst_engine import reset_shared_context
    ...
    debate_messages = run_debate(all_reports, dept_reports)
...
def show_help():
    console.print(Panel.fit(
        "[bold]AI Office — Gold Trading System[/]\n\n"
        "  python main.py          — Запуск цикла\n"
        "  python main.py --once   — Один цикл\n"
        "  python main.py --staff  — Список аналитиков\n"
        "  python main.py --web    — Веб-интерфейс (port 5000)\n"
        "  python main.py --stats  — Статистика трейдера\n"
        "  python main.py --history — История сделок\n"
        "  python main.py --it     — IT мониторинг\n"
        "  python main.py --reset  — Сброс баланса\n"
        "  python main.py --llm    — Проверка LLM\n"
        "  python main.py --help   — Справка\n\n"
        f"[bold cyan]LLM:[/] {'включён (Mistral)' if LLM_ENABLED else 'выключен'}",
        title="Gold Trading", border_style="yellow"
    ))
...
def run_forever():
    console.print(Panel.fit(
        f"[bold green]AI OFFICE — Gold Trading System[/bold green]\n"
        f"Instruments: {len(INSTRUMENTS)} | Staff: {len(ANALYSTS)} | TF: {TIMEFRAME} | Interval: {ANALYSIS_INTERVAL_MINUTES}m",
        border_style="yellow"
    ))
...
if __name__ == "__main__":
    ...
    try:
        if "--help" in args or "-h" in args:
            ...
            try:
                llm = get_llm()
                console.print(f"[green]LLM подключение: OK[/]")
                console.print(f"  Provider: {llm.provider}")
                console.print(f"  Model: {llm.model}")
            except Exception as e:
                console.print(f"[red]LLM ошибка: {e}[/]")
        elif "--it" in args:
            from it_monitor_IT import run_it_cycle
            run_it_cycle()
            console.print(f"[green]IT цикл завершён. Проверь it_log.md[/]")
        elif "--reset" in args:
            trades_file = os.path.join(BASE_DIR, "logs", "trade_history.json")

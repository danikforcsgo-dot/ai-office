...
from config import FOREX_PAIRS, TIMEFRAME, ANALYSIS_INTERVAL_MINUTES, LLM_ENABLED
...
def run_cycle():
    ...
    llm_status = "[bold green]LLM: OpenRouter[/]" if LLM_ENABLED else "[bold yellow]LLM: OFF (rule-based only)[/]"
    console.print(f"[bold]  {llm_status}[/bold]")
    console.print(f"[bold]{'='*60}[/bold]\n")

    if LLM_ENABLED:
        try:
            from config import OPENROUTER_FREE_MODELS
            console.print(f"[dim]  Доступно моделей: {len(OPENROUTER_FREE_MODELS)}[/dim]")
        except Exception:
            pass
...
def show_help():
    console.print(Panel.fit(
        ...
        "  python main.py --llm    — Проверка LLM подключения\n"
        "  python main.py --help   — Справка\n\n"
        f"[bold cyan]LLM:[/] {'включён (OpenRouter)' if LLM_ENABLED else 'выключен'}",
